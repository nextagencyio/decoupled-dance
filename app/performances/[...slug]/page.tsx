...
import { GET_PERFORMANCE_BY_PATH } from '@/lib/queries'
import { DrupalPerformance } from '@/lib/types'
...
interface PerformanceByPathData {
  route: {
    entity: DrupalPerformance
  } | null
}

async function getPerformance(path: string): Promise<DrupalPerformance | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<PerformanceByPathData>({
      query: GET_PERFORMANCE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/performances/${slug.join('/')}`
  const item = await getPerformance(path)

  if (!item) {
    return { title: 'Performance Not Found | Dance' }
  }

  return {
    title: `${item.title} | Dance`,
