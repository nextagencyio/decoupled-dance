...
export default function CTASection({ homepageContent }: CTASectionProps) {
  const title = (homepageContent as any)?.ctaTitle || 'Ready to Dance?'
  const description = (homepageContent as any)?.ctaDescription?.processed || ''
  const primaryLabel = (homepageContent as any)?.ctaPrimary || 'Join a Class'
  ...
  return (
    <section className="bg-accent-400 py-20 relative overflow-hidden">
      ...
      <div className="absolute bottom-8 right-[12%] w-12 h-12 bg-purple-400 rounded-2xl opacity-30 rotate-45 animate-bounce" style={{ animationDuration: '3.5s', animationDelay: '1s' }} />
